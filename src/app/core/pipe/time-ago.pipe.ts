...
export class TimeAgoPipe implements PipeTransform {
  ...
  transform(value: Date): string {
    ...
    if (interval > 1)
      return this.translate.instant('YEARS', { count: interval });
    if (interval === 1) return this.translate.instant('ONE_YEAR');

    interval = Math.floor(seconds / 2592000);
    if (interval > 1)
      return this.translate.instant('MONTHS', { count: interval });
    if (interval === 1) return this.translate.instant('ONE_MONTH');

    interval = Math.floor(seconds / 86400);
    if (interval > 1)
      return this.translate.instant('DAYS', { count: interval });
    if (interval === 1) return this.translate.instant('ONE_DAY');

    interval = Math.floor(seconds / 3600);
    if (interval > 1)
      return this.translate.instant('HOURS', { count: interval });
    if (interval === 1) return this.translate.instant('ONE_HOUR');

    interval = Math.floor(seconds / 60);
    if (interval > 1)
      return this.translate.instant('MINUTES', { count: interval });
