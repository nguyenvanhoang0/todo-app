...
import { IDayDetails } from '../../types/date.type';
...
export class CalendarService {
  ...
  calculateDaysInMonth(currentYear: number, currentMonth: number) {
    ...
    const firstDayOfWeek = (firstDay.getDay() + 6) % 7; // Chuyển Chủ nhật thành ngày cuối tuần
    ...
    for (let day = firstDay.getDate(); day <= lastDay.getDate(); day++) {
      const status = this.calculateDayStatus(
        day,
        currentMonth,
        currentYear,
        today
    ...
    while (this.weeks.length < 6) {
      ...
      const nextMonthDay = new Date(
        currentYear,
        currentMonth,
        lastDayOfLastWeek.day + 1
      );

      const remainingDays = 7 - daysInWeek.length;
      for (
        let day = nextMonthDay.getDate();
        day <= nextMonthDay.getDate() + remainingDays - 1;
        day++
      ) {
        const month = currentMonth === 12 ? 1 : currentMonth + 1;
        const year = currentMonth === 12 ? currentYear + 1 : currentYear;
